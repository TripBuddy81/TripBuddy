$(document).ready(function () {
    ...
    window.videoDromeDirectorDurationMinDefault = 2000;
    ...
    $('body').html(Handlebars.compile($('#mainTemplate').html())(config));
    ...
    $.ajax({
        type: 'GET', url: 'https://www.pornhub.com', success: function () {
    ...
    function rightMouseButtonAction(e) {
        ...
        if ($('.keyboard-cancel-button').is(':visible')) {
            ...
            if ($('#menuClose').prop('checked') || $('#quickTrackSelectionMenu').hasClass('menuTransition') || $('#applicationSettingsMenu').hasClass('menuTransition') || !$('#shrine').hasClass('shrineColorfulBackground') || $('#videodrome').is(':visible') || $('#notesOverlay').is(':visible') || $('.videoMenuOverlayFullscreen').is(':visible') || $('#preFlightChecklist').is(':visible') || $('.displayedFullscreenImage').is(':visible') || $('.keyboard-cancel-button').is(':visible') || $('#absoluteTruthsOverlay').is(':visible') || $('#relationships').is(':visible') || $('#OuijaYesNo').is(':visible') || $('#chatContainer').is(':visible') || $('#meditationSymbol').is(':visible') || shrineDiscoActive || stroboSpeed > 0 || showParticles) {
                stopAllActions();
            } else {
    ...
    $('#showPornZapper').click(function () {
        ...
        // videodromePlayInterval = setInterval( forcePlaybackVideodrome, 1000 ); // TODO REMOVE??
    });
    ...
    window.onYouTubePlayerAPIReady = function () {
        ...
        mainSearchResultYoutubePlayer = new YT.Player('mainSearchResultYoutubeIframe', {
            videoId: 'TdU2Ab7y91w', playerVars: {
                rel: 0,
                autoplay: 0,
                controls: 1,
                showinfo: 0,
    ...
    function startScreensaver(force = false) {
        ...
        $('.mainSectionActive').each(function () {
            if ($(this).attr('data-target') == 'videos' || $(this).attr('data-target') == 'images') {
                if (!$('#quickSelectGlobalMenuContainer').hasClass('menuTransition') && !$('#quickTrackSelectionMenu').hasClass('menuTransition') && !$('#applicationSettingsMenu').hasClass('menuTransition') && (screensaverSecondsIdle >= screensaverStartAfterSeconds || force) && !screensaverActive && !blockScreenSaver && !$('#menuClose').prop('checked')) {
    ...
    function nextRandomImageInSlideshow() {
        ...
        while (potentialNextImageIndex == currentImageIndex || '.' + config['images'][potentialNextImageIndex]['tags'] != imageTagList || jQuery.inArray(potentialNextImageIndex, shownImages) !== -1) {
            potentialNextImageIndex = Math.floor((Math.random() * config['images'].length));
        }
    ...
    $('.shrineSetBGColor').click(function (event) {
        ...
        $('#toggleRelationshipsWhite,#toggleAbsoluteThruthWhite,#shrineDiscoModeWhite,#shrineOuijaWhite,#shrineOracleWhite,#toggleThinkerWhite,#toggleChatWhite').show();
        $('#toggleRelationships,#toggleAbsoluteThruth,#shrineDiscoMode,#toggleAbsoluteThruthInwards,#toggleAbsoluteThruthOutwards,#shrineOuija,#shrineOracle,#toggleGospel,#toggleThinker,#toggleChat').hide();
        if (xxxVisible) {
    ...
    $('.shrineSetBGColorful').click(function (event) {
        ...
        $('#toggleRelationshipsWhite,#toggleAbsoluteThruthWhite,#shrineDiscoModeWhite,#toggleAbsoluteThruthInwardsWhite,#toggleAbsoluteThruthOutwardsWhite,#shrineOuijaWhite,#shrineOracleWhite,#toggleGospelWhite,#toggleThinkerWhite,#toggleChatWhite').hide();
        $('#toggleRelationships,#toggleAbsoluteThruth,#shrineDiscoMode,#shrineOuija,#shrineOracle,#toggleThinker,#toggleChat').show();
        if (xxxVisible) {
    ...
    $('#shrineSettingsContainer').hover(function () {
        $('#mainMenu').attr('style', 'opacity:1');
    }, function () {
    ...
    $('#toggleAbsoluteThruthContainer').hover(function (event) {
    ...
    $('#toggleChat,#toggleChatWhite').on('wheel click', function (event) {
        enableFullscreen();
    ...
    function nextChat() {
        count = 0;
        repitition = Math.floor(Math.random() * 42) + 1
        clearInterval(chatInterval);
    ...
    function openSpotifyApp() {
        if (config['spotifyMainPlayerName'] != '' && config['spotifyMainPlayerName'].indexOf('DESKTOP') >= 0 && localStorage.getItem('access_token') != null && typeof $('#devices option:contains("' + config['spotifyMainPlayerName'] + '")').val() == 'undefined' && spotifyOpened == false) {
            window.open(lastSelectedPlaylist, '_blank');
            spotifyOpened = true;
        } else if (config['spotifyMainPlayerName'] != '' && config['spotifyMainPlayerName'].indexOf('Web Player') >= 0 && localStorage.getItem('access_token') != null && typeof $('#devices option:contains("' + config['spotifyMainPlayerName'] + '")').val() == 'undefined' && spotifyOpened == false) {
        ...
        $.getScript('https://open.spotify.com/embed-podcast/iframe-api/v1', function (data, textStatus, jqxhr) {
            window.onSpotifyIframeApiReady = (IFrameAPI) => {
                ...
                let callback = (EmbedController) => {
                    document.querySelectorAll('.spotifyPlaylistItem').forEach(episode => {
    ...
    function searchYoutube(searchTerm, increaseApiKey = false) {
        ...
        $.ajax({
            type: 'GET', url: 'https://www.googleapis.com/youtube/v3/search', data: {
    ...
    function searchYoutubeSpecificVideoId(videoId, increaseApiKey = false) {
        ...
        $.ajax({
            type: 'GET', async: false, url: 'https://www.googleapis.com/youtube/v3/videos', data: {
                key: config['youtubeApiKey' + youtubeApiKeyInUse], id: videoId, part: 'snippet'
            }, success: function (searchYoutubeResult) {
    ...
    function getVideoDurationsFromYoutube(searchYoutubeResult) {
        ...
        $.ajax({
            type: 'GET', url: 'https://www.googleapis.com/youtube/v3/videos', data: {
                key: config['youtubeApiKey' + youtubeApiKeyInUse], part: 'contentDetails', id: listOfVideoIds
        ...
        $.each(youtubeCurrentQueue, function (key, value) {
            if (value.id != videoId || videoIdCount > 0) {
                tempYoutubeCurrentQueue.push({
                    'id': value.id, 'img': value.img, 'description': value.description, 'duration': value.duration
                });
            } else {
                videoIdCount++;
            }
    ...
    $(document).on('mousemove click', '.videodromeDirectorControlToBeHiddenIcon', function (e) {
        trueMouseClickDetected = false;

        if (e.type == 'click' && e.originalEvent != undefined) { // true click event detection
            trueMouseClickDetected = true;
        }
        clearTimeout(videodromeFullscreenMenuHideInterval);
    ...
    $(document).on('click', '#videodromeDirectorShuffleDeactivate', function (e) {
        e.preventDefault();
    ...
    $(document).on('click', '#videodromeDirectorLockVideoActivate', function (e) {
        e.preventDefault();
    ...
    $(document).on('click', '#videodromeDirectorLockVideoDeactivate', function (e) {
        e.preventDefault();
        $('#videodromeDirectorLockVideoActivate').show();
        $('#videodromeDirectorLockVideoDeactivate').hide();
        $('.videodromeDirectorVideoLocked').hide();
    ...
    function getNextVideoStreamUrl(newSearch = false, searchUrl = createSearchUrl(), retry = true, isSingleVideoPage = false) {
        ...
        if (activePageCrawls <= 2) {
            ...
            if ((newSearch || videoJSHubUrls.length <= 0) && !isSingleVideoPage) {
                $.ajax({
                    url: searchUrl, type: 'GET', success: function (originalData) {
            ...
            if ((videoJSSingleVideoUrls.length <= 4 && videoJSHubUrls.length >= 1) || isSingleVideoPage) {
                ...
                $.ajax({
                    url: singelVideoPageUrl, type: 'GET', success: function (data) {
                        switch (selectedVideoStreamService) {
                            case 'XV':
    ...
    function loadNextVideoJSStream(playerId) {
        if (videoJSSingleVideoUrls.length >= 1) {
            ...
            videoJSPlayer.src({
                src: singleVideoObject['videoStreamUrl'], type: 'application/x-mpegURL'
            });
    ...
    function getNextPrivatePictureDir() {
        ...
        $.ajax({
            url: privatePictureDirContainer['dirPath'], success: function (data) {
                $(data).find('td > a').each(function () {
                    tempFilename = $(this).attr('href');
                    if (tempFilename.indexOf('/') >= 0 && tempFilename != '/') {
                    } else if (tempFilename != '/') {
